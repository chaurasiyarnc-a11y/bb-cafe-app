...
import { 
  ...
  ChevronLeft, ChevronRight, Monitor, Bell, CheckCircle2, Tag, ChevronDown, ChevronUp
} from 'lucide-react';
...
const SafeChevronDown = ChevronDown as any;
const SafeChevronUp = ChevronUp as any;
...
export default function BbCafePosDesktop() {
  ...
  const handleAddCustomizedItemToCart = () => {
    ...
    setCart((prev) => {
      const existingIndex = prev.findIndex((c) => c.id === compositeId);
      if (existingIndex > -1) {
        const next = [...prev];
        next[existingIndex].quantity += 1;
        return next;
      }
      return [...prev, { id: compositeId, name: compositeName, price: normalPizzaPrice, quantity: 1 }];
    });
          ...
          <main className="flex-1 flex flex-col overflow-hidden bg-neutral-50 dark:bg-[#0a0a0a]">
            ...
            {activeTab === 'billing' && (
              <div className="flex-1 flex overflow-hidden">
                ...
                {/* Right Side: Fixed Desktop Cart Panel */}
                  ...
                  <div className="h-full flex flex-col overflow-y-auto p-5 space-y-4">
                    ...
                    <div className="pt-1">
                      <button 
                        ...
                        className="w-full flex items-center justify-between bg-neutral-100 dark:bg-neutral-800/60 hover:bg-neutral-200 dark:hover:bg-neutral-800 px-3 py-2 rounded-xl text-xs font-bold text-neutral-400 transition-all border border-neutral-200 dark:border-neutral-800"
                      >
                        <span className="flex items-center gap-2"><SafeTag size={14} className="text-orange-500" /> Add Discount / Coupon Code</span>
