...
import { 
  collection, onSnapshot, query, orderBy, limit, addDoc, where, getDocs 
...
export default function BbCafePosDesktop() {
  ...
  // --- PRINTER STATES (PC SPECIAL) ---
  const [printerDevice, setPrinterDevice] = useState<any>(null); // USB or Serial Device
  const [connectionType, setConnectionType] = useState<'usb' | 'serial' | 'none'>('none');

  // टोकन नंबर - कार्ट खाली होने पर नया आता है
  const tokenNumber = useMemo(() => Math.floor(100 + Math.random() * 900), [cart.length === 0]);

  // --- प्रिंटर कॉन्फ़िगरेशन (Utils के लिए) ---
  const pConfig: PrintConfig = { 
    printerPaperSize: '58mm', 
    printerType: connectionType === 'usb' ? 'thermal_usb' : 'thermal_bluetooth',
    usbDevice: printerDevice, // USB डिवाइस यहाँ पास होगा
    fontSize: 10
  } as any;

  // --- 1. USB प्रिंटर कनेक्ट करने का फंक्शन ---
  const connectUsbPrinter = async () => {
    try {
      if (!('usb' in navigator)) return toast.error("Browser doesn't support USB");
      ...
      toast.success(`USB Printer Connected: ${device.productName}`);
    } catch (err: any) {
      console.error(err);
      toast.error("USB Connection Failed. Try using a different USB Port.");
    }
  };
    ...
    return onSnapshot(q, (snap) => {
      ...
      const list = snap.docs.map(d => {
        ...
        if(data.status === 'completed') {
          t += (data.total || 0);
          if(data.paymentMethod === 'cash') c += (data.total || 0);
          if(data.paymentMethod === 'upi') u += (data.total || 0);
        }
        return { id: d.id, ...data };
      });
  ...
  // --- 4. आर्डर और प्रिंटिंग लॉजिक ---
  const handlePlaceOrder = async () => {
    if (cart.length === 0 || isSubmittingOrder) return;
    setIsSubmittingOrder(true);
    const toastId = toast.loading("Saving Order & Printing...");
    ...
    try {
      ...
      const orderObj = { 
        ...
        customerName: customerName || "Walk-in Guest", customerPhone: customerPhone ? `+91${customerPhone}` : "",
      ...
      // थर्मल प्रिंटर के लिए थोड़ा गैप
      ...
      toast.success(`Order Successful! Bill #${billNumber}`, { id: toastId });
      setCart([]); setCustomerName(''); setCustomerPhone('');
    } catch (err) {
      console.error(err);
      toast.error("Printing Failed. Check Printer Connection.", { id: toastId });
    } finally {
  ...
  if (!isLoggedIn) return (
    <div className="h-screen w-full bg-[#050505] flex items-center justify-center font-sans">
      <Toaster />
      <div className="bg-[#111] p-12 rounded-[50px] border border-white/5 w-[420px] text-center shadow-2xl">
        <Lock size={64} className="text-orange-500 mx-auto mb-8" />
        <h1 className="text-2xl font-black text-white mb-8 tracking-tighter uppercase">Bum Bum Terminal</h1>
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" className="w-full bg-orange-600 text-white font-black py-5 rounded-2xl uppercase shadow-xl shadow-orange-600/20 transition-all active:scale-95">Unlock POS</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="h-screen w-full bg-[#080808] text-white flex overflow-hidden font-sans select-none">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#111] flex flex-col shrink-0">
        <div className="p-8 border-b border-white/5">
          ...
          <p className="text-[10px] text-slate-500 font-bold uppercase mt-1 tracking-widest">Desktop Version</p>
        </div>
        ...
        <div className="p-6 border-t border-white/5 bg-[#0a0a0a]">
           <div className="bg-orange-500/10 p-4 rounded-2xl border border-orange-500/20 mb-4 text-center">
              <p className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Today's Sales</p>
      ...
      <main className="flex-1 flex overflow-hidden bg-[#000]">
        ...
        {activeTab === 'billing' && (
          <>
            <div className="flex-1 flex flex-col min-w-0">
              <header className="p-6 border-b border-white/5 bg-[#111]/50">
                <div className="relative">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" />
                  <input ref={searchInputRef} type="text" placeholder="Search dish... (Space)" className="w-full bg-[#1a1a1a] border-none rounded-2xl py-4 pl-12 text-lg outline-none focus:ring-1 ring-orange-500" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} />
                </div>
              </header>

              <div className="p-4 flex gap-2 overflow-x-auto no-scrollbar shrink-0 bg-[#0a0a0a]">
                {categories.map(c => <button key={c} onClick={()=>setSelectedCategory(c)} className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === c ? 'bg-orange-500 text-white' : 'bg-[#1a1a1a] text-slate-500 hover:text-white'}`}>{c}</button>)}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 content-start">
                {products.filter(p => (selectedCategory==='All'||p.category===selectedCategory)&&p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(item => (
                  <div key={item.id} onClick={()=>setCart([...cart, {...item, quantity: 1}])} 
                       className={`bg-[#111] border border-white/5 rounded-[40px] p-5 cursor-pointer hover:border-orange-500/50 transition-all group overflow-hidden active:scale-95 ${item.isAvailable === false ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                    <div className="aspect-[4/3] w-full bg-[#1a1a1a] rounded-[30px] mb-4 overflow-hidden">
                        {item.image ? <img src={item.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt={item.name} /> : <div className="w-full h-full flex items-center justify-center text-white/5 font-black text-2xl uppercase tracking-tighter italic">Bum Bum</div>}
                    </div>
              ...
              <div className="h-12 bg-orange-600 flex items-center px-10 gap-8 shrink-0 text-[10px] font-black uppercase">
                 <div className="flex items-center gap-2"><Keyboard size={16}/> Shortcuts:</div>
            ...
            <aside className="w-[480px] border-l border-white/5 bg-[#111] flex flex-col shadow-2xl">
              ...
              <div className="p-8 bg-[#151515] border-t border-white/5 space-y-6">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <input type="text" placeholder="Guest Name" className="bg-[#000] p-4 rounded-2xl outline-none text-sm font-bold border border-white/5 focus:ring-1 ring-orange-500" value={customerName} onChange={e=>setCustomerName(e.target.value)} />
                </div>
                <div className="flex bg-[#000] p-1.5 rounded-2xl">
                    ...
                    <button onClick={()=>setPaymentMethod('upi')} className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all ${paymentMethod==='upi'?'bg-blue-600 text-white shadow-lg shadow-blue-600/20':'text-slate-500 hover:text-white'}`}>UPI (F2)</button>
                </div>
                <div className="flex justify-between items-end border-t border-white/5 pt-4">
                    <span className="text-slate-500 font-black uppercase text-[10px] tracking-widest italic">Total Payable</span>
                ...
                <button disabled={isSubmittingOrder||cart.length===0} onClick={handlePlaceOrder} className="w-full bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white font-black py-5 rounded-[32px] text-xl shadow-2xl shadow-orange-600/20 active:scale-95 transition-all uppercase tracking-tighter">
                  {isSubmittingOrder ? <Loader2 className="animate-spin mx-auto" size={24} /> : "Complete & Print"}
                </button>
              </div>
            </aside>
          </>
        ...
        {activeTab === 'printer_setup' && (
          <div className="flex-1 p-12 overflow-y-auto space-y-10">
            <header>
               ...
               <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-2">Connect your Thermal USB Printer directly</p>
            </header>
            <div className="grid grid-cols-2 gap-10 max-w-5xl">
              <div className="bg-[#111] p-10 rounded-[50px] border border-white/5 space-y-6">
                 ...
                 <h3 className="text-xl font-black uppercase tracking-tight">Direct USB Connection</h3>
                 <p className="text-sm text-slate-500 italic">प्रिंटर केबल को PC से जोड़ें और 'Detect' बटन दबाएं।</p>
                 <button onClick={connectUsbPrinter} className="w-full bg-orange-600 font-black py-5 rounded-[25px] flex items-center justify-center gap-3 shadow-lg shadow-orange-600/20 hover:bg-orange-500 transition-all">
                    <Search size={20}/> DETECT USB PRINTER
                 </button>
              </div>
              <div className="bg-[#111] p-10 rounded-[50px] border border-white/5 flex flex-col justify-center text-center">
                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Connected Status</p>
                 <div className={`p-6 rounded-3xl flex flex-col items-center gap-4 ${printerDevice ? 'bg-green-600/10 text-green-500 border border-green-600/20' : 'bg-red-600/10 text-red-500 border border-red-600/20'}`}>
                    {printerDevice ? <Wifi size={40} /> : <X size={40} />}
                    <p className="text-xl font-black">{printerDevice ? printerDevice.productName : "NO PRINTER DETECTED"}</p>
                 </div>
                 {printerDevice && (
                    <button onClick={async () => {
                      const encoder = new TextEncoder();
                      const data = encoder.encode("\x1B\x40BUM BUM CAFE\nPRINTER WORKING OK\n\n\x1D\x56\x41\x03");
                      await printerDevice.transferOut(1, data);
                    }} className="mt-6 text-[10px] font-black uppercase text-orange-500 border border-orange-500/20 py-3 rounded-2xl hover:bg-orange-500 hover:text-white transition-all">Test Print</button>
                 )}
              </div>
            </div>
          </div>
        ...
        {activeTab === 'receipts' && (
           <div className="flex-1 p-12 overflow-y-auto space-y-4">
              <h2 className="text-3xl font-black italic mb-8">Billing History</h2>
              <div className="grid grid-cols-2 gap-6">
                {pastReceipts.map(order => (
                  <div key={order.id} className="bg-[#111] p-6 rounded-[35px] flex justify-between items-center border border-white/5 hover:border-orange-500/50 group transition-all">
                    <div className="flex gap-6">
                      <div className="bg-black/50 px-5 py-3 rounded-2xl text-center">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Token</p>
                        <p className="text-2xl font-black text-orange-500 font-mono italic leading-none mt-1">#{order.tokenNumber}</p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500 font-mono tracking-tighter mb-1">ID: #{order.billNumber}</p>
                        <p className="text-lg font-black italic">{order.customerName} - <span className="text-green-500">₹{order.total}</span></p>
                      </div>
                    </div>
                    <button onClick={() => handlePrintReceipt(order, pConfig)} className="bg-orange-600 p-4 rounded-2xl opacity-0 group-hover:opacity-100 shadow-lg active:scale-90 transition-all"><Printer size={22}/></button>
                  </div>
                ))}
              </div>
           </div>
