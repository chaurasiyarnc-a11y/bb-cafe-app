...
import { 
  ...
  LayoutDashboard, UserCircle, Smartphone, Tag
} from 'lucide-react';
...
export default function BumBumCafe_FinalPOS() {
  // --- States ---
  ...
  // --- 1. Printer Connection (Improved) ---
  const connectPrinter = async () => {
    if (!("serial" in navigator)) {
      toast.error("Please use Chrome or Edge browser for printing!");
      return;
    }
    try {
      ...
      setPrinterPort(port);
      toast.success("80mm USB Printer Ready!");
    } catch (e) {
      toast.error("Printer connection failed or cancelled.");
    }
  };

  // --- 2. Sequential Printing Logic (KOT + Bill) ---
  const handlePrint = async (order: any) => {
    ...
    try {
      ...
      b += "------------------------------------------\n";
      b += `Time: ${new Date().toLocaleTimeString()} | Dine-In\n`;
      b += "------------------------------------------\n";
      ...
      toast.error("Print Error! Check Printer.");
    } finally {
      writer.releaseLock();
    }
  };

  // --- 3. Save Order Logic (Fixed Transaction) ---
  const placeOrder = async () => {
    ...
    try {
      ...
      if (printerPort) {
        await handlePrint(orderData);
      } else {
        toast("Order Saved. Printer not connected.", { icon: '⚠️' });
      }

      setCart([]); setCustomerPhone(''); setCustomerName('Walk-in Guest');
    } catch (err) {
      console.error(err);
      toast.error("Order Failed! Database Error.", { id: toastId });
    } finally {
  ...
  if (!isLoggedIn) return (
    <div className="h-screen bg-black flex items-center justify-center">
      <div className="bg-neutral-900 p-10 rounded-[2.5rem] border border-neutral-800 w-96 text-center">
        <Utensils className="mx-auto text-orange-500 mb-6" size={48} />
        ...
        />
        <p className="text-neutral-500 text-xs">Login with PIN: 1234</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen flex bg-neutral-950 text-white overflow-hidden font-sans select-none">
      ...
      <nav className="w-24 border-r border-neutral-900 flex flex-col items-center py-8 gap-8">
        <div className="p-3 bg-orange-600 rounded-2xl shadow-lg shadow-orange-600/20"><Utensils size={28} /></div>
        ...
        <button onClick={connectPrinter} className={`mt-auto p-4 rounded-2xl ${printerPort ? 'bg-green-600 text-white' : 'bg-neutral-900 text-neutral-500'}`}><Wifi size={28}/></button>
        ...
        {activeTab === 'billing' ? (
            ...
            <header className="flex justify-between items-center mb-8 gap-6">
              <div className="relative flex-1 max-w-xl">
                ...
                <input 
                  ...
                  className="w-full bg-neutral-900 border border-neutral-800 rounded-2xl py-4 pl-14 pr-6 outline-none focus:border-orange-500 text-lg"
                  value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                />
              </div>
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {categories.map(c => (
                  <button key={c} onClick={() => setSelectedCategory(c)} className={`px-6 py-3 rounded-xl text-xs font-black uppercase whitespace-nowrap ${selectedCategory === c ? 'bg-orange-600 text-white' : 'bg-neutral-900 text-neutral-500'}`}>{c}</button>
                ))}
              </div>
            </header>

            <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 pr-2 scrollbar-hide">
              {products
                ...
                <button 
                  ...
                  className="bg-neutral-900 border border-neutral-800 rounded-[2.5rem] p-4 text-left hover:border-orange-500 transition-all active:scale-95 group"
                >
                  <div className="h-40 bg-neutral-800 rounded-3xl mb-4 overflow-hidden relative">
                    {(p.imageUrl || p.image) ? (
                      <img src={p.imageUrl || p.image} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-all duration-500" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center"><ImageIcon className="text-neutral-800" size={40} /></div>
                    )}
                  </div>
                  <h3 className="font-bold text-base truncate px-1">{p.name}</h3>
                  <p className="text-orange-500 font-black text-xl px-1">₹{p.price}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="p-8 grid grid-cols-3 gap-6 overflow-y-auto">
            {liveOrders.filter(o => o.status === 'pending').map(order => (
              <div key={order.id} className="bg-neutral-900 border border-neutral-800 rounded-[2rem] p-6 space-y-5 shadow-xl">
                <div className="flex justify-between items-center">
                  <span className="bg-orange-600 px-4 py-1 rounded-xl font-black text-xs">Token: #{order.tokenNumber}</span>
                  <span className="text-neutral-500 text-xs font-mono">{new Date(order.timestamp?.toDate()).toLocaleTimeString()}</span>
                </div>
      ...
      {/* RIGHT SIDEBAR: CART */}
      <aside className="w-[440px] bg-neutral-900/40 border-l border-neutral-900 flex flex-col p-8 shadow-2xl">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-2xl font-black uppercase tracking-tighter flex items-center gap-3"><ShoppingBag className="text-orange-500" size={28} /> CART</h2>
        ...
        <div className="bg-black/40 border border-neutral-800 rounded-3xl p-5 mb-6 space-y-4">
          <div className="flex items-center gap-3">
            <UserCircle className="text-neutral-500" size={24} />
            <input 
        ...
        <div className="flex-1 overflow-y-auto space-y-4 mb-6 pr-2 scrollbar-hide">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-neutral-800">
              ...
              <p className="font-black mt-4 uppercase tracking-widest text-sm">Cart is empty</p>
            </div>
          ) : cart.map(item => (
            <div key={item.id} className="bg-black/40 border border-neutral-800 p-5 rounded-3xl flex items-center justify-between group">
              <div className="flex-1 pr-4">
                <p className="font-bold text-sm leading-tight">{item.name}</p>
                <p className="text-orange-500 font-black mt-1">₹{item.price * item.quantity}</p>
              </div>
              <div className="flex items-center gap-4 bg-neutral-900 p-1.5 rounded-2xl">
                <button onClick={() => setCart(cart.map(i => i.id === item.id ? {...i, quantity: Math.max(1, i.quantity - 1)} : i))} className="p-2 hover:bg-neutral-800 rounded-xl"><Minus size={14}/></button>
                <span className="font-black text-base w-4 text-center">{item.quantity}</span>
                <button onClick={() => setCart(cart.map(i => i.id === item.id ? {...i, quantity: i.quantity + 1} : i))} className="p-2 hover:bg-neutral-800 rounded-xl"><Plus size={14}/></button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-neutral-800 space-y-6">
          <div className="flex justify-between items-end">
            ...
            <span className="text-4xl font-black text-white font-mono tracking-tighter">₹{cart.reduce((a, b) => a + (b.price * b.quantity), 0)}</span>
          </div>
          <button 
