...
export default function BbCafePosMobile() {
  ...
  // --- OPEN VARIATION MODAL FOR HALF/FULL/PLAIN/BUTTER ---
                      ...
                      {filteredMenu.map((item) => {
                        ...
                        return (
                          <motion.button 
                            ...
                            key={item.id} 
                            disabled={!isAvail} 
                            onClick={() => { triggerBeep('tap'); item.variants ? setSelectedProduct(item) : handleOpenVariationModal(item); }} 
          ...
          <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-white dark:bg-neutral-900 rounded-t-3xl p-5 max-h-[90vh] flex flex-col shadow-2xl overflow-y-auto">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1 mb-3">
                {cart.map((item, idx) => (
                  <div key={idx} className="bg-neutral-50 dark:bg-neutral-800/30 border border-neutral-200 dark:border-neutral-800 p-2.5 rounded-2xl flex items-center justify-between gap-2">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-xs truncate">{item.name}</p>
                      {item.note && <p className="text-[10px] text-neutral-400 italic truncate">Note: {item.note}</p>}
                      <p className="text-[11px] font-mono text-orange-500">₹{item.price * item.quantity}</p>
                    </div>
